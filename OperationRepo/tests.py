...
class SearchTest(TestCase):
    ...
    def test_search_singleword(self):
        request = Request("http://0.0.0.0:7000/operationrepo/search/?format=json&search=blah")
    ...
    def test_search_tempty(self):
        request = Request('http://0.0.0.0:7000/operationrepo/search/?format=json&search=""')
        response = urlopen(request)
        self.assertEqual(response.getcode(), 200)
        content = loads(response.readall().decode("utf-8"))
    ...
    def test_search_multiple(self):
        request = Request('http://0.0.0.0:7000/operationrepo/search/?format=json&search=blah+pool')
        response = urlopen(request)
        self.assertEqual(response.getcode(), 200)
        content = loads(response.readall().decode("utf-8"))
        print(content)
